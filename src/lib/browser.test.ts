...
import { getUnsupportedBrowserReason, isMobileDevice } from './browser'
...
describe('isMobileDevice', () => {
  ...
  it('does not treat desktop browsers as mobile', () => {
    ...
    expect(isMobileDevice(fakeNavigator(userAgents.desktopFirefox))).toBe(false)
  })
})
...
describe('getUnsupportedBrowserReason', () => {
  ...
  it('accepts desktop Firefox, which exposes Web MIDI', () => {
    expect(
      getUnsupportedBrowserReason(fakeNavigator(userAgents.desktopFirefox), true),
    ).toBeUndefined()
  })
