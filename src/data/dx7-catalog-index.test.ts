...
describe('DX7 catalog index', () => {
  ...
  it('matches the patch names and voice fingerprints in every catalog bank file', async () => {
    const index = Object.fromEntries(
      dx7BankCatalog.map((bank) => {
        const file = readFileSync(resolve('public', bank.file.slice(1)))
        return [
          bank.id,
          parseDx7Bank(Uint8Array.from(file).buffer).map(({ data, name }) => [
            name,
            voiceFingerprint(data),
          ]),
        ]
      }),
    )
