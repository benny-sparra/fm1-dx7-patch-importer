//#region node_modules/webmidi/dist/esm/webmidi.esm.js
/**
* WEBMIDI.js v3.1.16
...
var EventEmitter = class EventEmitter {
	...
	*/
	hasListener(event, callback) {
		...
		} else if (this.eventMap[event] && this.eventMap[event].length > 0) {
			if (callback instanceof Listener) return this.eventMap[event].filter((listener) => listener === callback).length > 0;
			else if (typeof callback === "function") return this.eventMap[event].filter((listener) => listener.callback === callback).length > 0;
			else if (callback != void 0) return false;
			return true;
		} else return false;
	}
	/**
...
var Listener = class {
	...
	*/
	constructor(event, target, callback, options = {}) {
		...
		if (typeof callback !== "function") throw new TypeError("The 'callback' must be a function.");
		if (options.arguments !== void 0 && !Array.isArray(options.arguments)) options.arguments = [options.arguments];
...
* @since 3.0.0
*/
var Enumerations = class Enumerations {
	/**
...
var Note = class {
	...
	*/
	constructor(value, options = {}) {
		...
		if (options.rawRelease != void 0) this.release = Utilities.from7bitToFloat(options.rawRelease);
...
var Utilities = class {
	...
	*/
	static toNoteNumber(identifier, octaveOffset = 0) {
		...
		if (fragments.accidental) if (fragments.accidental.startsWith("b")) result -= fragments.accidental.length;
	...
	*/
	static sanitizeChannels(channel) {
		let channels;
		if (wm.validation) {
			if (channel === "all") channels = ["all"];
			else if (channel === "none") return [];
		}
		if (!Array.isArray(channel)) channels = [channel];
	...
	*/
	static guessNoteNumber(input, octaveOffset) {
		octaveOffset = parseInt(octaveOffset) || 0;
		let output = false;
		if (Number.isInteger(input) && input >= 0 && input <= 127) output = parseInt(input);
		else if (parseInt(input) >= 0 && parseInt(input) <= 127) output = parseInt(input);
		else if (typeof input === "string" || input instanceof String) try {
	...
	*/
	static buildNote(input, options = {}) {
		...
		if (number === false) throw new TypeError(`The input could not be parsed as a note (${input})`);
	...
	*/
	static getCcNumberByName(name) {
		...
		else return Enumerations.MIDI_CONTROL_CHANGE_MESSAGES[name];
	}
	/**
	...
	*/
	static getChannelModeByNumber(number) {
		...
		for (let cm in Enumerations.CHANNEL_MODE_MESSAGES) if (Enumerations.CHANNEL_MODE_MESSAGES.hasOwnProperty(cm) && number === Enumerations.CHANNEL_MODE_MESSAGES[cm]) return cm;
		return false;
	}
	/**
...
var OutputChannel = class extends EventEmitter {
	...
	*/
	sendKeyAftertouch(target, pressure, options = {}) {
		if (wm.validation) {
			...
			} else if (!(pressure >= 0 && pressure <= 1)) throw new RangeError("Key aftertouch value must be a float between 0 and 1.");
		}
	...
	*/
	sendControlChange(controller, value, options = {}) {
		...
		if (wm.validation) {
			if (controller === void 0) throw new TypeError("Control change must be identified with a valid name or an integer between 0 and 127.");
	...
	*/
	sendRpnDecrement(parameter, options = {}) {
		...
		if (wm.validation) {
			if (parameter === void 0) throw new TypeError("The specified registered parameter is invalid.");
			let valid = false;
	...
	*/
	playNote(note, options = {}) {
		...
		for (let note of notes) if (parseInt(note.duration) > 0) {
	...
	*/
	sendNoteOff(note, options = {}) {
		if (wm.validation) {
			if (options.rawRelease != void 0 && !(options.rawRelease >= 0 && options.rawRelease <= 127)) throw new RangeError("The 'rawRelease' option must be an integer between 0 and 127");
			if (options.release != void 0 && !(options.release >= 0 && options.release <= 1)) throw new RangeError("The 'release' option must be an number between 0 and 1");
		...
		let nVelocity = 64;
		if (options.rawRelease != void 0) nVelocity = options.rawRelease;
	...
	*/
	sendNoteOn(note, options = {}) {
		if (wm.validation) {
			if (options.rawAttack != void 0 && !(options.rawAttack >= 0 && options.rawAttack <= 127)) throw new RangeError("The 'rawAttack' option must be an integer between 0 and 127");
			if (options.attack != void 0 && !(options.attack >= 0 && options.attack <= 1)) throw new RangeError("The 'attack' option must be an number between 0 and 1");
			if (options.rawVelocity) {
		...
		let nVelocity = 64;
		if (options.rawAttack != void 0) nVelocity = options.rawAttack;
	...
	*/
	sendChannelMode(command, value = 0, options = {}) {
		...
		if (wm.validation) {
			if (command === void 0) throw new TypeError("Invalid channel mode message name or number.");
	...
	*/
	sendOmniMode(state, options = {}) {
		if (state === void 0 || state) this.sendChannelMode("omnimodeon", 0, options);
	...
	*/
	sendChannelAftertouch(pressure, options = {}) {
		if (wm.validation) {
			...
			} else if (!(pressure >= 0 && pressure <= 1)) throw new RangeError("Channel aftertouch value must be a float between 0 and 1.");
		}
		if (!options.rawValue) pressure = Utilities.fromFloatTo7Bit(pressure);
		this.send([(Enumerations.CHANNEL_MESSAGES.channelaftertouch << 4) + (this.number - 1), Math.round(pressure)], { time: Utilities.toTimestamp(options.time) });
		return this;
	}
	/**
	...
	*/
	sendModulationRange(semitones, cents, options = {}) {
		if (wm.validation) {
			...
			if (!(cents == void 0) && (!Number.isInteger(cents) || !(cents >= 0 && cents <= 127))) throw new RangeError("If specified, the cents value must be an integer between 0 and 127.");
		}
	...
	*/
	sendPitchBend(value, options = {}) {
		if (wm.validation) if (options.rawValue && Array.isArray(value)) {
	...
	*/
	sendProgramChange(program, options = {}) {
		...
		this.send([(Enumerations.CHANNEL_MESSAGES.programchange << 4) + (this.number - 1), program], { time: Utilities.toTimestamp(options.time) });
		return this;
	}
	/**
...
var Output = class extends EventEmitter {
	...
	*/
	sendTimecodeQuarterFrame(value, options = {}) {
		...
		this.send([Enumerations.SYSTEM_MESSAGES.timecode, value], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendSongSelect(value = 0, options = {}) {
		...
		this.send([Enumerations.SYSTEM_MESSAGES.songselect, value], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendTuneRequest(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.tunerequest], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendClock(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.clock], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendStart(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.start], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendContinue(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.continue], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendStop(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.stop], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendActiveSensing(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.activesensing], { time: options.time });
		return this;
	}
	/**
	...
	*/
	sendReset(options = {}) {
		this.send([Enumerations.SYSTEM_MESSAGES.reset], { time: options.time });
		return this;
	}
	/**
	* @private
	...
	constructor(destinations = [], options = {}) {
		...
		*/
		...
		if (wm.validation) {
			...
			if (options.types !== void 0) options.types.forEach((type) => {
			...
			if (options.channels !== void 0) options.channels.forEach((channel) => {
	...
	*/
	_parseEventForStandardMessages(e) {
		...
		if (event.type === "noteoff" || event.type === "noteon" && data2 === 0) {
			...
			* and 127).
			*/
			event.note = new Note(Utilities.offsetNumber(data1, this.octaveOffset + this.input.octaveOffset + wm.octaveOffset), { rawAttack: data2 });
			event.value = Utilities.from7bitToFloat(data2);
			event.rawValue = data2;
	...
	*/
	_parseEventForParameterNumber(event) {
		...
		if (controller === 99 || controller === 101) {
			this._nrpnBuffer = [];
			this._rpnBuffer = [];
			if (controller === 99) this._nrpnBuffer = [event.message];
			else if (value !== 127) this._rpnBuffer = [event.message];
		} else if (controller === 98 || controller === 100) if (controller === 98) {
			this._rpnBuffer = [];
			if (this._nrpnBuffer.length === 1) this._nrpnBuffer.push(event.message);
			else this._nrpnBuffer = [];
		} else {
			this._nrpnBuffer = [];
			if (this._rpnBuffer.length === 1 && value !== 127) this._rpnBuffer.push(event.message);
			else this._rpnBuffer = [];
		}
		else if (controller === 6 || controller === 38 || controller === 96 || controller === 97) if (this._rpnBuffer.length === 2) this._dispatchParameterNumberEvent("rpn", this._rpnBuffer[0].dataBytes[1], this._rpnBuffer[1].dataBytes[1], event);
	...
	*/
	constructor(data) {
		...
		*/
		...
		if (this.statusByte === Enumerations.SYSTEM_MESSAGES.sysex) if (this.dataBytes[0] === 0) {
	...
	*/
	_onMidiMessage(e) {
		...
		*/
		...
		if (message.isSystemMessage) this._parseEvent(event);
		else if (message.isChannelMessage) this.channels[message.channel]._processMidiMessageEvent(event);
...
var WebMidi = class extends EventEmitter {
	...
	*/
	getInputById(id, options = { disconnected: false }) {
		...
		if (options.disconnected) {
			for (let i = 0; i < this._disconnectedInputs.length; i++) if (this._disconnectedInputs[i]._midiInput && this._disconnectedInputs[i].id === id.toString()) return this._disconnectedInputs[i];
		} else for (let i = 0; i < this.inputs.length; i++) if (this.inputs[i]._midiInput && this.inputs[i].id === id.toString()) return this.inputs[i];
	}
	/**
	...
	*/
	getInputByName(name, options = { disconnected: false }) {
		...
		if (options.disconnected) {
			for (let i = 0; i < this._disconnectedInputs.length; i++) if (~this._disconnectedInputs[i].name.indexOf(name)) return this._disconnectedInputs[i];
		} else for (let i = 0; i < this.inputs.length; i++) if (~this.inputs[i].name.indexOf(name)) return this.inputs[i];
	}
	/**
	...
	*/
	getOutputByName(name, options = { disconnected: false }) {
		...
		if (options.disconnected) {
			for (let i = 0; i < this._disconnectedOutputs.length; i++) if (~this._disconnectedOutputs[i].name.indexOf(name)) return this._disconnectedOutputs[i];
		} else for (let i = 0; i < this.outputs.length; i++) if (~this.outputs[i].name.indexOf(name)) return this.outputs[i];
	}
	/**
	...
	*/
	getOutputById(id, options = { disconnected: false }) {
		...
		if (options.disconnected) {
			for (let i = 0; i < this._disconnectedOutputs.length; i++) if (this._disconnectedOutputs[i]._midiOutput && this._disconnectedOutputs[i].id === id.toString()) return this._disconnectedOutputs[i];
		} else for (let i = 0; i < this.outputs.length; i++) if (this.outputs[i]._midiOutput && this.outputs[i].id === id.toString()) return this.outputs[i];
	}
	/**
	* @private
	...
	_onInterfaceStateChange(e) {
		...
		*/
		...
		if (e.port.state === "connected" && e.port.connection === "open") {
			...
			if (e.port.type === "input") event.port = this.getInputById(e.port.id, { disconnected: true });
			else if (e.port.type === "output") event.port = this.getOutputById(e.port.id, { disconnected: true });
			this.emit(e.port.state, event);
			const portsChangedEvent = Object.assign({}, event);
			portsChangedEvent.type = "portschanged";
			this.emit(portsChangedEvent.type, portsChangedEvent);
	...
	*/
	async _updateInputs() {
		...
		for (let i = this._inputs.length - 1; i >= 0; i--) {
			...
			if (!Array.from(this.interface.inputs.values()).find((input) => input === current._midiInput)) {
	...
	*/
	async _updateOutputs() {
		...
		for (let i = this._outputs.length - 1; i >= 0; i--) {
			...
			if (!Array.from(this.interface.outputs.values()).find((output) => output === current._midiOutput)) {
