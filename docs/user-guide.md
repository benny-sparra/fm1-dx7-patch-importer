...
The search box above the patch grid looks through every bank that has patches in it, not only the one shown. It matches a patch's name or its slot code, such as `B07`, and lists the matches in bank order, each labelled with its slot. Click a result to play it as you would in its bank; the results stay up so you can try the next one. Choosing a bank on the left clears the search and shows that bank.
